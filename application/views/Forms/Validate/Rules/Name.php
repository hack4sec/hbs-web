 ...
 * @author Anton Kuzmin
 */
class Forms_Validate_Rules_Name extends Forms_Validate_Abstract {
    ...
    protected $_messageTemplates = array(
        self::REPEAT => 'L_RULES_LIST_ALREADY_EXISTS'
    );
    ...
    public function repeat($title) {
        ...
        if($this->_exclude) {
            $editGroup = $Rules->get($this->_exclude);
            if($title == $editGroup['name'])
                return false;
        }

        if($Rules->exists($title)) {
